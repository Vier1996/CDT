...
{
    public class CatPetSitSubBehavior : CatRelaxingSubBehavior
    {
        ...
        public CatPetSitSubBehavior(RelaxPoint relaxPoint, CatComponents catComponents) : base(relaxPoint, catComponents)
        {
        ...
        {
            _catAnimator.SetAnimation(CatAnimationType.rest_sit_to, completeCallback: LaunchWorkTimer);
        }

        private void LaunchWorkTimer()
        {
            _workingDisposable?.Dispose();
            _workingDisposable = RX.Delay(7f, TryReUpdateSubBehavior);
            
            _catAnimator.SetAnimation(CatAnimationType.action_pet_sit, 
                completeCallback: () => _catAnimator.SetAnimation(CatAnimationType.rest_sit_idle));
        }
