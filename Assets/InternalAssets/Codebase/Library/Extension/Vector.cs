...
namespace InternalAssets.Codebase.Library.Extension
{
    public static class Vector
    {
        public static Vector3 GetPointInRadius(this Vector3 from, float minRadius, float maxRadius)
        {
            ...
            float x = from.x + UnityEngine.Random.Range(minRadius, maxRadius) * Mathf.Cos(randomAngle * Mathf.Deg2Rad);
            float y = from.y + UnityEngine.Random.Range(minRadius, maxRadius) * Mathf.Sin(randomAngle * Mathf.Deg2Rad);
